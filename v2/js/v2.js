...
(function () {
  ...
  (function () {
    ...
    add("bot", "Hola, soy Ampli, el agente de inteligencia artificial de Amplifia. Cuéntame qué desafío enfrenta tu organización y te indico cómo podemos acompañarla.");
  ...
  function build() {
    ...
    function sgFit() {
      ...
      var size = cap;
      vts.forEach(function (v) { size = Math.min(size, 100 * avail / (sgCv.measureText(v.textContent).width || 1) * 0.97); });
      size = Math.max(16, size).toFixed(1) + "px";
      vts.forEach(function (v) { v.style.fontSize = size; });
    }
    sgFit();
